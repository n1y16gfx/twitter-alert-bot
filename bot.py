...
# --- KEYWORDS EN + FR (variantes apostrophes incluses) ---
KEYWORDS = [
...
while True:
    try:
        ...
        print("Query envoyée :", query)
        response = client.search_recent_tweets(query=query, max_results=10)

        if response.data:
            print(f"{len(response.data)} tweet(s) récupéré(s)")
            for tweet in response.data:
                print("Tweet trouvé :", tweet.text)
                if tweet.id in seen_tweets:
                    continue
                seen_tweets.add(tweet.id)
                message = f"🚨 CLIENT POTENTIEL 🚨\n\n{tweet.text}\n\nhttps://twitter.com/i/web/status/{tweet.id}"
                send_telegram_alert(message)
        else:
            print("Aucun tweet trouvé pour le moment.")
